...
class MediaServiceOld : Service(), MediaPlayer.OnPreparedListener, MediaPlayer.OnErrorListener,
    ...
    private fun createRunnable(media: MediaServiceOld): TimerTask = object : TimerTask() {
        override fun run() {
    ...
    class SessionCallbackReceiver(private val service: MediaServiceOld) : MediaSessionCompat.Callback(){
        override fun onPlay() {
            super.onPlay()
        }
    ...
    class IntentReceiver(private val service: MediaServiceOld) : BroadcastReceiver() {
    ...
    class QueueListener(private val mediaController: MediaServiceOld) : Response.Listener<String> {
