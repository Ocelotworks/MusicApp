...
class FastScroller(
    ...
    val verticalTrackDrawable: Drawable,
    ...
    fun show() {
        when (mAnimationState) {
            ANIMATION_STATE_FADING_OUT -> {
                ...
                mAnimationState =
                    ANIMATION_STATE_FADING_IN
                mShowHideAnimator.setFloatValues(
                    mShowHideAnimator.animatedValue as Float,
                    1f
                )
...
// before showing it back.
    ...
    fun updateScrollPosition(offsetX: Int, offsetY: Int) {
        ...
        if (mNeedVerticalScrollbar) {
            ...
            mVerticalThumbHeight = Math.max(
                defaultWidth * 4, Math.min(
        ...
        if (mNeedHorizontalScrollbar) {
            ...
            mHorizontalThumbWidth = Math.max(
                defaultWidth * 4, Math.min(
    ...
    override fun onInterceptTouchEvent(
        ...
        if (mState == STATE_VISIBLE) {
            val insideVerticalThumb =
                isPointInsideVerticalThumb(ev.x, ev.y)
            val insideHorizontalThumb =
                isPointInsideHorizontalThumb(ev.x, ev.y)
    ...
    override fun onTouchEvent(
        ...
        if (me.action == MotionEvent.ACTION_DOWN) {
            val insideVerticalThumb =
                isPointInsideVerticalThumb(me.x, me.y)
            val insideHorizontalThumb =
                isPointInsideHorizontalThumb(me.x, me.y)
    ...
    private fun verticalScrollTo(y: Float) {
        ...
        y1 = scrollbarRange[0].coerceAtLeast(min(scrollbarRange[1], y.toInt())).toFloat()
        if (abs(mVerticalThumbCenterY - y1) < 2) {
            return
        }
        val scrollingBy = scrollTo(
            mVerticalDragY, y1, scrollbarRange,
            mRecyclerView!!.computeVerticalScrollRange(),
            mRecyclerView!!.computeVerticalScrollOffset(), mRecyclerViewHeight
        )
        if (scrollingBy != 0) {
            mRecyclerView!!.scrollToPosition(y1.toInt())
        }
        mVerticalDragY = y1
    }
    ...
    private fun horizontalScrollTo(x: Float) {
        ...
        x = scrollbarRange[0].coerceAtLeast(min(scrollbarRange[1], x.toInt())).toFloat()
    ...
    private inner class AnimatorListener : AnimatorListenerAdapter() {
        ...
        override fun onAnimationEnd(animation: Animator) { // Cancel is always followed by a new directive, so don't update state.
            if (mCanceled) {
                mCanceled = false
                return
            }
    ...
    init {
        mVerticalThumbWidth =
            Math.max(defaultWidth, mVerticalThumbDrawable.intrinsicWidth)
        mVerticalTrackWidth =
            Math.max(defaultWidth, verticalTrackDrawable.intrinsicWidth)
